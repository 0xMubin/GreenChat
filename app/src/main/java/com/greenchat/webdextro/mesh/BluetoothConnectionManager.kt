...
class BluetoothConnectionManager(
     ...
     */
    fun broadcastPacket(packet: GreenchatPacket) {
        if (!isActive) return
        ...
        val serverCallback = object : BluetoothGattServerCallback() {
            ...
            override fun onCharacteristicWriteRequest(
                ...
                if (characteristic.uuid == CHARACTERISTIC_UUID) {
                    val packet = GreenchatPacket.fromBinaryData(value)
                    if (packet != null) {
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        Log.d(TAG, "Processing greenchat device: $deviceAddress, name: '$deviceName', peerID: $extractedPeerID, RSSI: $rssi")
        ...
        synchronized(pendingConnections) {
            ...
            if (extractedPeerID != null) {
                ...
                Log.i(TAG, "Initiating connection to device with greenchat service at $deviceAddress (RSSI: $rssi, attempt: $attempts)")
            }
...
interface BluetoothConnectionManagerDelegate {
    fun onPacketReceived(packet: GreenchatPacket, peerID: String, device: BluetoothDevice?)
