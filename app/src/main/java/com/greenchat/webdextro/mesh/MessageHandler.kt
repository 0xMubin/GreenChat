...
class MessageHandler(private val myPeerID: String) {
     ...
     */
    suspend fun handleAnnounce(packet: GreenchatPacket, peerID: String): Boolean {
     ...
     */
    suspend fun handleMessage(packet: GreenchatPacket, peerID: String) {
     ...
     */
    private suspend fun handleBroadcastMessage(packet: GreenchatPacket, peerID: String) {
        try {
            // Parse message
            val message = GreenchatMessage.fromBinaryPayload(packet.payload)
            if (message != null) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: GreenchatPacket, peerID: String) {
        try {
            ...
            // Parse message
            val message = GreenchatMessage.fromBinaryPayload(unpaddedData)
            if (message != null) {
                // Check for cover traffic (dummy messages)
                if (message.content.startsWith("☂DUMMY☂")) {
     ...
     */
    suspend fun handleLeave(packet: GreenchatPacket, peerID: String) {
     ...
     */
    suspend fun handleDeliveryAck(packet: GreenchatPacket, peerID: String) {
     ...
     * Handle read receipt
     */
    suspend fun handleReadReceipt(packet: GreenchatPacket, peerID: String) {
        if (packet.recipientID != null && String(packet.recipientID).replace("\u0000", "") == myPeerID) {
            try {
                val decryptedData = delegate?.decryptFromPeer(packet.payload, peerID)
                if (decryptedData != null) {
     ...
     */
    private suspend fun relayMessage(packet: GreenchatPacket) {
     ...
     */
    private fun sendDeliveryAck(message: GreenchatMessage, senderPeerID: String) {
        handlerScope.launch {
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: GreenchatPacket)
    fun relayPacket(packet: GreenchatPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: GreenchatPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: GreenchatMessage)
