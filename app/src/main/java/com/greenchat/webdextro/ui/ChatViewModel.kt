...
import com.greenchat.webdextro.mesh.BluetoothMeshDelegate
import com.greenchat.webdextro.mesh.BluetoothMeshService
...
/**
 * Refactored ChatViewModel - Main coordinator for greenchat functionality
...
class ChatViewModel(application: Application) : AndroidViewModel(application), BluetoothMeshDelegate {
    ...
    val messages: LiveData<List<GreenchatMessage>> = state.messages
    ...
    val privateChats: LiveData<Map<String, List<GreenchatMessage>>> = state.privateChats
    ...
    val channelMessages: LiveData<Map<String, List<GreenchatMessage>>> = state.channelMessages
    ...
    private fun loadAndInitialize() {
        ...
        viewModelScope.launch {
            ...
            if (state.getConnectedPeersValue().isEmpty() && state.getMessagesValue().isEmpty()) {
                val welcomeMessage = GreenchatMessage(
                    sender = "system",
                    content = "👋 Ask nearby friends to install the app and start chatting — no internet required. Made with ❤️ by Muntasir Mubin",
                    timestamp = Date(),
                    isRelay = false
                )
    ...
    override fun didReceiveMessage(message: GreenchatMessage) {
    ...
    fun panicClearAllData() {
        ...
        // Reset nickname
        val newNickname = "alien${Random.nextInt(1000, 9999)}"
        state.setNickname(newNickname)
        dataManager.saveNickname(newNickname)
